    ...
    async function load() {
      ...
      try {
        const res = await fetch(
          `/api/reviews?patientId=${encodeURIComponent(patientId)}&pageSize=10&sort=newest`
        );
    ...
    <div className="max-w-5xl mx-auto p-6 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <Link href="/" className="px-3 py-2 rounded border bg-white hover:bg-gray-50 shadow-sm">
            Home
          </Link>
          <Link
      ...
      {error && (
        <div className="mb-4 bg-red-50 text-red-700 px-3 py-2 rounded-md text-sm border border-red-200">
          {error}
        </div>
      )}
            ...
            <div className="divide-y">
              {reviews.map((r) => {
                ...
                return (
                  <div
                    key={r.id}
                    className="py-3 flex flex-col md:flex-row md:items-center md:justify-between gap-2 hover:bg-gray-50 px-2 rounded-md"
                  >
                    <div>
                      ...
                      <div className="text-xs text-gray-500 mt-0.5">
                        Doctor ID: <span className="font-mono">{r.doctorId}</span> · Appointment:{" "}
                    ...
                    <div className="flex items-center gap-2">
                      <Link
                        href={editHref}
                        className="px-3 py-1.5 rounded-md bg-indigo-600 text-white text-sm hover:bg-indigo-700 shadow"
                      >
                        View / Edit
                      </Link>
                    </div>
