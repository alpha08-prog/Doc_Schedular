...
export default function DoctorReviewsPage({
  ...
  const [distribution, setDistribution] = useState<{ [k: number]: number }>({
    1: 0,
    2: 0,
    3: 0,
    4: 0,
    ...
    <div className="max-w-5xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <Link href="/doctor/dashboard" className="px-3 py-2 rounded border hover:bg-gray-50">
            Dashboard
          </Link>
          <Link href="/doctor/patients" className="px-3 py-2 rounded border hover:bg-gray-50">
            Patients
          </Link>
        </div>
      </div>
      ...
      {error && (
        <div className="mb-4 bg-red-50 text-red-700 px-3 py-2 rounded text-sm">{error}</div>
      )}
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="text-sm text-gray-600">
          Total Reviews: <span className="font-semibold text-gray-900">{total}</span>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1">
            ...
            <select
              value={ratingMin}
              onChange={(e) =>
                setRatingMin(e.target.value === "" ? "" : parseInt(e.target.value, 10))
              }
          ...
          <div className="flex items-center gap-1">
            ...
            <select
              value={ratingMax}
              onChange={(e) =>
                setRatingMax(e.target.value === "" ? "" : parseInt(e.target.value, 10))
              }
              className="border rounded px-2 py-1 text-sm"
            >
              <option value="">Any</option>
            ...
            >
              <div>
                ...
                <div className="text-xs text-gray-500 mt-1">
                  Patient: {r.patientName || r.patientId} · Appointment:{" "}
                  <span className="font-mono">{r.appointmentId}</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Link
                  href={`/doctor/patients/${r.patientId}/history`}
