...
export default function MedicalHistoryPage({ params }: { params: { patientId: string } }) {
  ...
  const timeline = useMemo(() => {
    ...
    prescriptions.forEach((p) =>
      items.push({ type: "prescription", date: p.prescriptionDate, item: p })
    );
  ...
  const totalAppointments = useMemo(
    () => appointments.filter((a) => inRange(a.date)).length,
    [appointments, from, to]
  );
  const totalPrescriptions = useMemo(
    () => prescriptions.filter((p) => inRange(p.prescriptionDate)).length,
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-lg border border-white/20 p-4">
            ...
            <p className="text-2xl font-bold text-purple-600">
              {diagnoses.filter((d) => inRange(d.date)).length}
            </p>
          </div>
        </div>
                    ...
                    <div className="flex-1">
                      ...
                      {entry.type === "appointment" && (
                        <div className="mt-1 text-gray-700">
                          <p>
                            <span className="font-medium">Reason:</span> {entry.item.reason}
                          </p>
                          {entry.item.notes && (
                            <p className="text-sm text-gray-600">{entry.item.notes}</p>
                          )}
                        </div>
                      )}
                      {entry.type === "diagnosis" && (
                        <div className="mt-1 text-gray-700">
                          <p>
                            <span className="font-medium">Diagnosis:</span> {entry.item.diagnosis}
                          </p>
                          {entry.item.notes && (
                            <p className="text-sm text-gray-600">{entry.item.notes}</p>
                          )}
                      ...
                      {entry.type === "prescription" && (
                        <div className="mt-1 text-gray-700">
                          ...
                          <p className="text-sm">
                            {entry.item.dosage} • {entry.item.duration}
                          </p>
                          {entry.item.notes && (
                            <p className="text-sm text-gray-600">{entry.item.notes}</p>
                          )}
